...
{0x0B,0x00,0x00,0x00,0x00,0x00},{0x0C,0x00,0x00,0x00,0x00,0x00},
{0x0D,0x00,0x00,0x00,0x00,0x00},{0x0E,0x00,0x00,0x00,0x00,0x00},
...
uint8_t bufferToFrame(uint8_t *RecvBuffer,uint8_t lens,struct plcFrameStruct *plcRecvFrame,struct uartControlU uartCtl)
{
    ...
    uint8_t crc=0;
//    checkBufferValidOrInvalid(RecvBuffer,lens,uartCtl);
//    if(uartCtl.recvValid==1)
//    {
        for(pos=0;pos<30;pos++){
            if(*(RecvBuffer+pos)==0x68&&*(RecvBuffer+pos+7)==0x68)
            {
             //   plcRecvFrame->isLocked = true;
...
void picApplyFrame(uint8_t picApplyState)
{
...
{ 
 switch (plcCtl.picApplyPlcState)
    ...
    case ApplyForVoltage:
            ...
            //apply for 10ms,if failed then again;
//            if(msCount-picApplyLastTime>500)
//            {
//                plcCtl.plcApplyPlcSuccess=0;
//            }
